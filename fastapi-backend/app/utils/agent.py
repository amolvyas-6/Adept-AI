...
class LLM:
    def __init__(self, rag_utility: RAGUtility):
        ...
        self.chat_model = ChatGroq(
            model=CHAT_MODEL, api_key=SecretStr(os.getenv("GROQ_API_KEY", ""))
        )
    ...
    def query(
        ...
        chat_history_modified = []
        for i in range(
            max(0, len(chat_history) - self.CHAT_HISTORY_LIMIT), len(chat_history)
        ):
            message: dict = chat_history[i]
            if message["role"] == "assistant":
                chat_history_modified.append(
                    AIMessage(
        ...
        human_message = HumanMessage(content=human_message_content)  # type: ignore
        for chunk in self.chat_model.stream(
            [self.system_message] + chat_history_modified + [human_message]
        ):
